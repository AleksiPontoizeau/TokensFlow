...
export async function readSnapshotHistory(options = {}) {
  ...
  try {
    ...
    if (error?.code === "ENOENT") {
      return {
        ...result,
        status: "empty",
        message: "No local history recorded yet."
      };
    }
    ...
    return {
      ...result,
      status: "error",
      message: `Cannot read local history: ${error?.code || "read error"}.`
    };
  }
  ...
  return {
    ...
    message: snapshots.length > 0 ? "Local history loaded." : "No history points in the last 24h.",
