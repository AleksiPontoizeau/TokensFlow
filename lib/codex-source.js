...
export const CODEX_PLUS_INPUT_USD_PER_1K = 0.003;
export const CODEX_PLUS_OUTPUT_USD_PER_1K = 0.015;
const FALLBACK_INPUT_SHARE = 0.9;
const FALLBACK_OUTPUT_SHARE = 0.1;
...
export async function readCodexUsage(options = {}) {
  ...
  const goal = thread ? await selectGoal(config.goalsDb, thread.id) : null;
  ...
  const localUsedTokens = positiveInteger(
    ...
    positiveInteger(goal?.tokens_used, positiveInteger(thread?.tokens_used, totalActivityTokens(activity)))
  );
  const basePayload = {
    ...
    retailCostUsd: activity.todayCostUsd,
    todayCostUsd: activity.todayCostUsd,
    last24hCostUsd: activity.last24hCostUsd,
    ...
    debug: {
      ...
      burnRateSkippedReason: "Codex quota is read from local rate_limits events; local token totals are not used to compute quota runway.",
      costPricing: `Codex Plus estimate: input $${CODEX_PLUS_INPUT_USD_PER_1K}/1K, output $${CODEX_PLUS_OUTPUT_USD_PER_1K}/1K`
    }
  };
  ...
  if (rateLimitSnapshot?.rateLimits?.primary || rateLimitSnapshot?.rateLimits?.secondary) {
    ...
    return normalizeUsage({
      ...basePayload,
      source: `codex local file · ${rateLimitSnapshot.rateLimits.plan_type || "plan"}`,
  ...
  return normalizeUsage({
    ...basePayload,
      source: `codex-local · quota unavailable · ${thread ? trimTitle(thread.title) : "no project thread"}`,
...
export async function buildCodexActivity24h(sessionsRoot, options = {}) {
  ...
  const todayKey = localDateKey(windowEnd, timezone);
  ...
  for (const file of files) {
    ...
    for (const line of text.split(/\n/)) {
      ...
      bucket.events += 1;
      bucket.costUsd += activity.costUsd;
      last24hCostUsd += activity.costUsd;

      if (localDateKey(eventDate, timezone) === todayKey) {
...
function tokenUsageDetails(usage) {
  ...
  const splitAvailable = inputTokens > 0 || outputTokens > 0;
  if (explicitTotal > 0) {
    return {
  ...
  const totalTokens = inputTokens + outputTokens + positiveInteger(usage.reasoning_output_tokens, 0);
  return {
    totalTokens,
    inputTokens,
    outputTokens,
...
function totalActivityTokens(activity) {
  if (!activity || !Array.isArray(activity.hourlyBuckets24h)) {
    return 0;
  }

  return activity.hourlyBuckets24h.reduce((total, bucket) => total + positiveInteger(bucket.usedTokens, 0), 0);
}

function estimateCodexPlusCost({ totalTokens, inputTokens, outputTokens, splitAvailable }) {
  const billableInputTokens = splitAvailable ? inputTokens : Math.round(totalTokens * FALLBACK_INPUT_SHARE);
  const billableOutputTokens = splitAvailable ? outputTokens : Math.round(totalTokens * FALLBACK_OUTPUT_SHARE);

  return (
    (billableInputTokens / 1000) * CODEX_PLUS_INPUT_USD_PER_1K
    + (billableOutputTokens / 1000) * CODEX_PLUS_OUTPUT_USD_PER_1K
  );
}
